...
template<typename Element>
std::string
encodeArgumentHead(Element argument)
{
	throw std::invalid_argument(
		"Non-specialized template parameter specified. Template parameter must be specialized to use this function.");
}
...
template<typename Element>
std::string
encodeArgumentTail(std::string encoding, size_t index, Element argument)
{
	throw std::invalid_argument(
		"Non-specialized template parameter specified. Template parameter must be specialized to use this function.");
}
...
template<typename Element, typename... Elements>
std::string
argumentEncodingTail(std::string encoding, size_t index, Element argument, Elements... arguments)
{
...
template<typename Element, typename... Elements>
std::string
argumentEncoding(Element argument, Elements... arguments)
{
...
template<typename Element, typename... Elements>
std::tuple<Element, Elements...>
decode(std::string_view encoding)
{
...
std::string
encode(std::string functionSignature, Elements... arguments)
{
	std::string encoding = "0x";

	if(!functionSignature.empty())
		encoding += Implementation::functionSelector(functionSignature);
